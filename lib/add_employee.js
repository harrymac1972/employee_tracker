...
async function get_managers_array() {
    let query_str = "SELECT * FROM employees WHERE manager_id IS NOT NULL";
    try {
        const db = db_conn.get_DB_object();
...
async function get_manager_id(manager_full_name) {    
    try {
        ...
        const query_str = "SELECT * FROM employees WHERE first_name = ? AND last_name = ?";
        const [rows, fields] = await db.promise().query(query_str, [first_name, last_name]);
        let id_n = rows[0]["id"];
        return id_n;
    } catch (error) {
        throw error;
